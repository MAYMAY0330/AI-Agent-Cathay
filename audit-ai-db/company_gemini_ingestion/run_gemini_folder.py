...
from company_gemini_ingestion.run_gemini_ingestion import run_gemini_ingestion
...
def run_gemini_folder_ingestion(
    folder_path: str,
    *,
    source_system: str,
    language: str,
    ...
    results: list[GeminiBatchResult] = []
    supported_seen = 0
    for file_path in all_files:
        ...
        if extension not in SUPPORTED_EXTENSIONS:
            ...
            batch_result = GeminiBatchResult(
                file_path=file_path,
...
def _result_from_run(
    ...
    return GeminiBatchResult(
        ...
        gemini_calls=int(result.get("gemini_calls") or 0),
...
def print_results(results: list[GeminiBatchResult]) -> None:
    counts: dict[str, int] = {}
...
def print_single_result(result: GeminiBatchResult) -> None:
    detail = [
        f"status={result.status}",
        f"stage={result.stage}",
        f"type={result.document_type or '-'}",
        f"chunks={result.total_chunks}",
        f"calls={result.gemini_calls}",
    ...
    print("GEMINI_FILE_RESULT " + " ".join(detail), flush=True)


def write_report(results: list[GeminiBatchResult], report_path: str) -> Path:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run Gemini parsing/chunking for every supported PDF/DOCX file in a folder."
    )
    parser.add_argument(
        "folder",
        nargs="?",
    ...
    parser.add_argument("--source-system", default="gemini_folder_test")
    parser.add_argument("--language", default="zh-TW")
    parser.add_argument("--internal-code-prefix", default="GEMINI-BATCH")
    parser.add_argument(
        "--output-dir",
    ...
    parser.add_argument(
        ...
        default=10,
        help=(
            "Skip a PDF if more than this many pages would need Gemini Vision. "
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="List files and inferred metadata without calling Gemini or writing the database.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--report-path",
        default="data/processed/gemini_ingestion_report.csv",
        help="CSV report output path. Default: data/processed/gemini_ingestion_report.csv",
    )
    parser.add_argument(
        "--no-report",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        results = run_gemini_folder_ingestion(
            args.folder,
